...
const Form = ({ users, setUsers, updatedData, count, setCounter, setToggle, setUpdatedData }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = (data) => {
    if (updatedData) {
      ...
      setUsers((prev) =>
        prev.map((val) =>
          val.id === updatedData.id ? { ...data, id: updatedData.id } : val,
        ),
      );
      return;
    }

    setUsers([...users, { ...data, id: count }]);
    reset();
    setCounter(count + 1);
  };
    ...
    <div className="p-4 border flex flex-col gap-3 text-white border-green-500 bg-gray-800 rounded-md mt-3 shadow-[0_0_15px_rgba(0,255,136,0.3),0_0_30px_rgba(0,255,136,0.5),inset_0_0_15px_rgba(0,255,136,0.15)]">
      ...
      >
        ...
        <p className="text-xs text-white/60 -mt-1">
          Tip: Fill all fields to add user.
        </p>
      </form>
    </div>
  );
